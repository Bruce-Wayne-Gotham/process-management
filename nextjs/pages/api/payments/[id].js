...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const result = await query(joinSql, [id]);
    ...
    if (req.method === 'PUT') {
      const { payment_date, amount_paid, payment_mode, transaction_ref, remarks } = await req.json();
      ...
      const result = await query(joinSql, [id]);
      return res.status(200).json(normalizePaymentRow(result.rows?.[0]));
    }
