...
export default async function handler(req, res) {
  ...
  try {
    const { start_date, end_date, payment_mode, farmer_id, group_by = 'date' } = req.query;

    const conditions = [];
    const params = [];
    let idx = 1;

    if (start_date) { conditions.push(`pay.payment_date >= $${idx++}`); params.push(start_date); }
    if (end_date) { conditions.push(`pay.payment_date <= $${idx++}`); params.push(end_date); }
    if (payment_mode) { conditions.push(`pay.payment_mode = $${idx++}`); params.push(payment_mode); }
    if (farmer_id) { conditions.push(`p.farmer_id = $${idx++}`); params.push(farmer_id); }
    ...
    const paymentModeBreakdown = payments.reduce((acc, payment) => {
      ...
      if (!acc[mode]) acc[mode] = { mode, count: 0, totalAmount: 0 };
      acc[mode].count++;
    ...
    if (group_by === 'date') {
      groupedData = payments.reduce((acc, payment) => {
        ...
        if (!acc[date]) acc[date] = { date, count: 0, totalAmountPaid: 0, totalPurchaseAmount: 0, payments: [] };
        acc[date].count++;
      ...
      groupedData = payments.reduce((acc, payment) => {
        ...
        if (!acc[farmerName]) acc[farmerName] = { farmerName, farmerCode: payment.purchases?.farmers?.farmer_code, village: payment.purchases?.farmers?.village, count: 0, totalAmountPaid: 0, totalPurchaseAmount: 0, payments: [] };
      ...
      groupedData = payments.reduce((acc, payment) => {
        const mode = payment.payment_mode || 'Unknown';
        if (!acc[mode]) acc[mode] = { mode, count: 0, totalAmountPaid: 0, totalPurchaseAmount: 0, payments: [] };
        acc[mode].count++;
    ...
    Object.keys(groupedData).forEach(key => {
      ...
      g.pendingAmount = g.totalPurchaseAmount - g.totalAmountPaid;
      g.paymentPercentage = g.totalPurchaseAmount > 0 ? (g.totalAmountPaid / g.totalPurchaseAmount) * 100 : 0;
    });

    return res.status(200).json({
      summary: {
        totalPayments, totalAmountPaid, totalPurchaseAmount, pendingAmount,
        ...
        dateRange: { start: start_date || 'All time', end: end_date || 'All time' }
      },
      groupBy: group_by,
