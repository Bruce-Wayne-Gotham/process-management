...
export default async function handler(req, res) {
  ...
  try {
    const { start_date, end_date, farmer_id, village, group_by = 'date' } = req.query;
    ...
    if (village) { conditions.push(`f.village LIKE $${idx++}`); params.push(`%${village}%`); }
    ...
    if (group_by === 'date') {
      groupedData = purchases.reduce((acc, purchase) => {
        ...
        if (!acc[date]) acc[date] = { date, count: 0, totalAmount: 0, totalWeight: 0, purchases: [] };
        acc[date].count++;
      ...
      groupedData = purchases.reduce((acc, purchase) => {
        ...
        if (!acc[farmerName]) acc[farmerName] = { farmerName, farmerCode: purchase.farmers?.farmer_code, village: purchase.farmers?.village, efficacyScore: purchase.farmers?.efficacy_score, count: 0, totalAmount: 0, totalWeight: 0, purchases: [] };
      ...
      groupedData = purchases.reduce((acc, purchase) => {
        ...
        if (!acc[villageKey]) acc[villageKey] = { village: villageKey, count: 0, totalAmount: 0, totalWeight: 0, farmers: new Set(), purchases: [] };
    ...
    return res.status(200).json({
      summary: { totalPurchases, totalAmount, totalWeight, avgRatePerKg, dateRange: { start: start_date || 'All time', end: end_date || 'All time' } },
      groupBy: group_by,
