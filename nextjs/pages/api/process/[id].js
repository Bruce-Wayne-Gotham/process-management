...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const result = await query(joinSql('WHERE p.id = $1'), [Number(id)]);
  ...
  if (req.method === 'PUT') {
    try {
      const { status_id, kadi_mati_weight, dhas_weight, remarks } = await req.json();
      ...
      if (status_id) {
        ...
        if (!statusCheck.rows?.[0]) return res.status(400).json({ error: 'Invalid status' });
      }
      ...
      if (status_id !== undefined) { sets.push(`status_id = $${++idx}`); values.push(Number(status_id)); }
      if (kadi_mati_weight !== undefined) { sets.push(`kadi_mati_weight = $${++idx}`); values.push(Number(kadi_mati_weight) || 0); }
      if (dhas_weight !== undefined) { sets.push(`dhas_weight = $${++idx}`); values.push(Number(dhas_weight) || 0); }

      if (sets.length > 1) {
        await query(
          `UPDATE process SET ${sets.join(', ')} WHERE id = $1 RETURNING id`,
          [Number(id), ...values]
        );
      }
